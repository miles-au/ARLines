...
THIS LIST IS EASY, CHECK IT OFF ONE-BY-ONE BABY! ✔️ ✔️ ✔️
 ...
 - [ ] Open the project in Xcode and add features to ARLines
...
If you are already using [CocoaPods](http://cocoapods.org), just add 'ARLines' to your `Podfile` then run `pod install`.
...
```ogdl
github "miles-au/ARLines" ~> 0.1
```

Then run `carthage update` to build the framework and drag the built `ARLines`.framework into your Xcode project.
...
ARLines is available under the MIT license. See [the LICENSE file](LICENSE) for more information.
